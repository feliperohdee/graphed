const _ = require('lodash');
const {
    Observable
} = require('rxjs');

module.exports = class Base {
    getKeys(namespace, pattern) {
        return this.redis.wrapObservable(this.redis.client.zrangebylex, `${namespace}:index`, `[${pattern}`, `[${pattern}\\xff`);
    }

    getRange(fromNodeId, start = 0, stop = -1) {
        return this.redis.wrapObservable(this.redis.client.zrange, fromNodeId, start, stop, 'WITHSCORES')
            .map((value, index) => {
                const isIndex = index % 2;

                return isIndex ? parseFloat(value) : value;
            })
            .bufferCount(2);
    }

    getRangeByScore(fromNodeId, range = [], limit = [], desc = false) {
        let [
            min = '-inf',
            max = '+inf'
        ] = range;

        let [
            offset = 0,
            count = 50
        ] = limit;

        if (desc) {
            [min, max] = [max, min];
        }

        return this.redis.wrapObservable(desc ? this.redis.client.zrevrangebyscore : this.redis.client.zrangebyscore, fromNodeId, min, max, 'WITHSCORES', 'LIMIT', offset, count)
            .map((value, index) => {
                const isIndex = index % 2;

                return isIndex ? parseFloat(value) : value;
            })
            .bufferCount(2);
    }

    getScore(fromNodeId, toNode) {
        return this.redis.wrapObservable(this.redis.client.zscore, fromNodeId, toNode)
            .map(parseFloat);
    }

    countFromSet(fromNodeId, min = '-inf', max = '+inf') {
        return this.redis.wrapObservable(this.redis.client.zcount, fromNodeId, min, max)
            .defaultIfEmpty(0);
    }

    addToSet(fromNodeId, toNode, value) {
        return Observable.forkJoin(
                this.index(fromNodeId),
                this.redis.wrapObservable(this.redis.client.zadd, fromNodeId, value, toNode)
            )
            .map(([
                a,
                b
            ]) => b);
    }

    index(fromNodeId) {
        const [
            namespace
        ] = fromNodeId.split(':');

        return Observable.forkJoin(
                this.redis.wrapObservable(this.redis.client.zadd, `${namespace}:index`, 0, fromNodeId),
                this.redis.wrapObservable(this.redis.client.zincrby, `${namespace}:indexUses`, 1, fromNodeId)
            )
            .map(([
                a,
                b
            ]) => {
                return Number(b);
            });
    }

    unindex(fromNodeId) {
        const [
            namespace
        ] = fromNodeId.split(':');

        return this.redis.wrapObservable(this.redis.client.zincrby, `${namespace}:indexUses`, -1, fromNodeId)
            .mergeMap(response => {
                response = Number(response);

                if (response <= 0) {
                    return Observable.forkJoin(
                            this.redis.wrapObservable(this.redis.client.zrem, `${namespace}:index`, fromNodeId),
                            this.redis.wrapObservable(this.redis.client.zrem, `${namespace}:indexUses`, fromNodeId)
                        )
                        .mapTo(response);
                }

                return Observable.of(response);
            })
    }

    removeFromSet(fromNodeId, ...keys) {
        return Observable.forkJoin(
                this.unindex(fromNodeId),
                this.redis.wrapObservable(this.redis.client.zrem, fromNodeId, ...keys)
            )
            .map(([
                a,
                b
            ]) => b);
    }

    incrementOnSet(fromNodeId, toNode, value, initial = 1) {
        return this.redis.wrapObservable(this.redis.client.zincrby, fromNodeId, value, toNode)
            .map(parseFloat)
            .mergeMap(response => {
                const isInitial = response === value;

                if (isInitial) {
                    return this.addToSet(fromNodeId, toNode, initial + value)
                        .mapTo(initial + value);
                }

                return Observable.of(response);
            });
    }
}